...
// Standard RFP Teams (Adjust names if your database has them spelled differently)
const TEAM_CATEGORIES = [
  {
    groupName: "PULPIT & PLATFORM",
    teams: ['Worship Team', 'Media & Tech', 'Decor']
  },
  {
    groupName: "GUEST EXPERIENCE",
    teams: ['Ushers', 'Protocol', 'Medical', 'Transport']
  },
  {
    groupName: "MINISTRY OPERATIONS",
    teams: ['Follow Up', 'Intercession', 'Childrens Ministry', 'Security']
  },
  {
    groupName: "OTHER",
    teams: ['Unassigned or No Team']
  }
];

// Locked to UTC Noon to prevent live timezone shifts
...
export default function TeamsMatrixDashboard() {
  ...
  // Uses the exact same localStorage keys as the Zones Matrix to keep them synced!
  ...
  // Download Handler Targetting Teams Matrix Container
  const handleDownload = async () => {
    ...
    try {
      const canvas = await html2canvas(element, { 
        ...
        useCORS: true,
        onclone: (clonedDoc) => {
          const clonedContainer = clonedDoc.getElementById('teams-matrix-export-container');
          if (clonedContainer) {
  ...
  // Bulletproof mapping logic across duplicates
  ...
  const isTeamMatch = (dbTeam: string, targetTeam: string) => {
    ...
    if (target === 'unassigned or no team' && (db === '' || db.includes('unassigned') || db.includes('n/a') || db === 'none' || db === 'n/a')) {
      return true;
    }
  ...
  return (
    <div className="w-full max-w-7xl flex flex-col gap-4">
      ...
      <div id="teams-matrix-export-container" className="bg-[#fefce8] p-4 md:p-6 rounded-2xl w-full flex flex-col gap-4 font-sans text-gray-900 border border-yellow-200 shadow-xl">
        ...
        <div className={`${theme.primaryBg} text-white p-5 rounded-xl flex justify-between items-center shadow-md relative overflow-hidden`}>
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-black uppercase tracking-widest text-emerald-200">Executive Ministry Report</span>
            </div>
            ...
            <h2 className="text-[#facc15] text-sm font-black tracking-widest mt-1">
              {MONTHS[selectedMonth - 1].toUpperCase()} {selectedYear} — {activeServiceType.toUpperCase()} TEAMS MATRIX
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
                ...
                <div key={wkNum} className={`bg-white rounded-lg shadow-sm border border-gray-200/80 overflow-hidden transition-all ${!isAvailable && 'opacity-50 grayscale bg-gray-50'}`}>
                  ...
                  <div className="p-2 space-y-1.5">
                    ...
                    <div className="flex justify-between items-center text-gray-700 px-1">
                      <span className="text-sm font-bold">System Registered</span>
          ...
          {/* RIGHT COLUMN: Grouped Matrix Table */}
              ...
              <table className="w-full text-left border-collapse font-medium">
                <thead>
                  <tr className="bg-gray-900 text-white border-b border-gray-800 text-xs">
                    <th className="py-2 px-3 uppercase tracking-wider font-black">TEAM / DEPARTMENT</th>
                    {displayWeeks.map(wk => (
                  ...
                  {TEAM_CATEGORIES.map((group, gIdx) => (
                    <Fragment key={gIdx}>
                      ...
                      {group.teams.map((teamName, tIdx) => {
                        ...
                        return (
                          <tr key={tIdx} className="hover:bg-gray-50/80 transition-colors border-b border-gray-100 text-gray-800">
                            <td className="py-1.5 px-3 font-bold text-sm text-gray-900">{teamName}</td>
